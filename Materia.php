...
public function MuestraMaterias($id_maestro)
{
    echo "<div class=table-responsive alig='center'>";
    echo "<table class=\"table table-striped\">";
    $sql_="SELECT m_m.id_maestro, m_m.id_materia, m.id,m.nombre FROM maestro_materia AS m_m ,materia AS m WHERE m_m.id_maestro=$id_maestro AND m_m.id_materia=m.id";
    $consult_=mysql_query($sql_) or die ("Error $sql_");
    while($field_ = mysql_fetch_array($consult_)){
    echo "<tr><td>".$Nombre = $field_['nombre']."</td></tr>";
